...
- The bar is shown on the first `onProgressChanged` callback where `webView.progress > 0`. Before the value hits the screen, it gets adjusted — if the raw progress is below 50%, it's clamped to 50% (`FIXED_PROGRESS`) so the bar immediately animates to at least 50% and never looks nearly empty.
